...
void zajmij_semafor(int konto) {
    ...
    if (konto == 3) {
        while(semafor1 == 0 && semafor2 == 0){
            pthread_cond_wait(&cond_both, &mutex);
        }
        semafor1 =0;
        semafor2 = 0;
    } else if (konto == 1){
...
void* klient(void* arg) {
    ...
    while (i < liczba_operacji) {
        ...
        if (operacja == 0) { 
            if(numer_konta == 3){
                printf("Id przelewu do konta1 %d. Dostalem %d zl\n", i, wartosc_operacji);
                zajmij_semafor(numer_konta);
                stankonta1 += wartosc_operacji;
                stankonta2 -= wartosc_operacji;
                printf("przelew %d zakończony. Nowe saldo konta 1: %d zł. Nowe saldo konta 2: %d\n", i, stankonta1, stankonta2);
                zwolnij_semafor(numer_konta);
            } else if (numer_konta == 1){
            ...
            if(numer_konta == 3){
                printf("Id przelewu do konta2 %d. Dostalem %d zl\n", i, wartosc_operacji);
                zajmij_semafor(numer_konta);
                stankonta2 += wartosc_operacji;
                stankonta1 -= wartosc_operacji;
                printf("przelew %d zakończony. Nowe saldo konta 1: %d zł. Nowe saldo konta 2: %d\n", i, stankonta1, stankonta2);
...
int main(int argc, char *argv[]) {
    ...
    if (pthread_cond_init(&cond_both, NULL) != 0) {
        perror("Zmienna warunkowa 2 nie została zainicjalizowana");
        return 1;
    }
    ...
    struct KlientArguments przelew;
    przelew.konto = 3;
    wyplata1.liczba_operacji = 1;
    wplata1.wartosc_operacji = 10;
